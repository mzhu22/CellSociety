...
public class Main extends Application {
	...
	public static final int PARAM_CONTROL_BOX_SPACING = 10;
	public static final int PARAM_CONTROL_BOX_WIDTH = 150;
	...
	private void makeMenu() {
		...
		// This allows you to load XML files. A class XMLReader handles this
		// task, returns a CASettings object containing
	...
	public void makeControlBox() {
		// Initialize the simulation controls (play/pause, speed slider, jumpTo,
		// etc.)
		myControls = new HBox();
	...
	private void makeParamControls() {
		myParamControls = new VBox();
		myParamControls.setSpacing(PARAM_CONTROL_BOX_SPACING);
		myParamControls.setMinWidth(PARAM_CONTROL_BOX_WIDTH);
		myParamControls.setFillWidth(false);
		myParamControls.setAlignment(Pos.TOP_CENTER);

		// TODO : Add a way to sort this - Brian Bolze
		for (String s:CASettings.myParameters.keySet()){
			TextField textField = new TextField((String)CASettings.myParameters.get(s));
			textField.setPrefWidth(TEXTFIELD_WIDTH);
			Label label = new Label(s);
			label.setFont(Font.font(12.));
			myParamControls.getChildren().add(label);
			myParamControls.getChildren().add(textField);
		}
		...
		myParamControls.getChildren().add(button);
	}

	/**
	 * Handles controlBox controls (button, slider, textField) Pressing button
	 * plays/pauses the simulation Dragging the slider increases/decreases
	 * animation speed Entering a number into the TextField currently does
	 * nothing
	 */
	...
	private void readXML() {
		...
		if (myGrid == null)
			return;
		if (!CASettings.myParameters.isEmpty()) {
			makeParamControls();
			myRoot.setRight(myParamControls);
		} else {
			myRoot.getChildren().remove(myParamControls);
		}
		myRoot.setCenter(myGrid);
	}
	 ...
	 * http://stackoverflow
	 * .com/questions/18669209/javafx-what-is-the-best-way-to
	...
	public static void showPopupMessage(final String message) {
		...
		popup.setOnShown(new EventHandler<WindowEvent>() {
			...
			public void handle(WindowEvent e) {
				popup.setX(myStage.getX() + myStage.getWidth() / 2
						- popup.getWidth() / 2);
				popup.setY(myStage.getY() + myStage.getHeight() / 2
						- popup.getHeight() / 2);
			}
		});
		popup.show(myStage);
